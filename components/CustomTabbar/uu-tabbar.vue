<template>
    ...
    <!-- 自定义tabbar布局 -->
    <view class="custom-tabbar__content safe-area-inset-bottom">
      <view v-for="(item, index) in list" :key="index" class="custom-tabbar__content__item" @tap.stop="clickHandler(index)">
        <view v-if="item.midButton" class="custom-tabbar__content__item__mid">
          <u-icon :name="elIconPath(index)" size="104rpx" img-mode="scaleToFill"></u-icon>
        </view>
        <view v-else class="custom-tabbar__content__item__normal">
          <u-icon :name="elIconPath(index)" size="48rpx" img-mode="scaleToFill"></u-icon>
          <text class="custom-tabbar__content__item__normal__text" :style="{ color: elColor(index) }">{{ item.text }}</text>
        </view>
      </view>
    </view>
		<view class="custom-tabbar--fixed-placeholder safe-area-inset-bottom"></view>
	</view>
</template>
...
export default {
	...
	methods: {
      ...
      if(clickItem.midButton){
        this.$emit("mid", clickItem)
      }
			if (clickItem.pagePath) {
				let url = clickItem.pagePath;
				uni.switchTab({
					url,
					fail: (err) => {
