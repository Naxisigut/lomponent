<template>
	<view>
		<view class="confirm-content cursor" @click="showModal">
			<text class="ellipsis" style="color: #000000;" v-if="title">{{title}}</text>
			<text class="ellipsis" v-else>{{placeholder}} 
				<view style="display: inline-block;"><u-icon name="arrow-right" color="#999999" size="14"></u-icon></view> 
			</text>
		</view>
		<view class="cascader-modal" :class="{show:isShowModal}" @click="hideModal">
			<view class="cascader-dialog" @click.stop="">
				<view class="cascader-title">
					...
					<text class="cursor" @click.stop="hideModal('hide')">
						<up-icon name="close" color="#000" size="28"></up-icon>
					</text>
				</view>
				<view class="cascader-tags">
					<view @click.stop="switchDate(item,index)" class="item cursor"
  ...
  methods: {
    ...
    select(item) {
      if (this.currentData.length && item.index < this.currentData.length - 1) {
        this.saveList.splice(item.index + 1);
        this.currentData.splice(item.index + 1);
        this.currentTitles.splice(item.index + 1);
      }

      this.$set(this.currentData, item.index, item.data);
      this.$set(this.currentTitles, item.index, item.data[this.labelKey]);
      if(item.index < 2){
        this.selectIndex = item.index + 1
      }
      if (item.data.children?.length) {
        this.currentList = this.setIndex(item.data.children, item.index);
        if (!this.currentTitles[item.index + 1]) {
          this.currentTitles[item.index + 1] = "未选择";
        }
      } else {
        this.saveList.splice(item.index + 1);
        this.currentTitles.splice(item.index + 1);
      ...
      this.$emit('change', this.currentData.map(it => it[this.valueKey]));
      if (!item.data.children?.length) this.hideModal();
    },
    /**
     * 切换数据 
     **/
    switchDate(item,index) {
      ...
      this.currentTitles.splice(index + 1);
      this.saveList.splice(index + 1);

      if (this.currentList.some(item => item.data.children?.length)) {
        this.currentTitles[index + 1] = "未选择";
      }
    },
    /**
     * 清空 
